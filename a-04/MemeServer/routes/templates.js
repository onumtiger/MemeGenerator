...
const templates = [ //hardcoded response from the get_memes API
    {
      "id": "181913649",
...
router.get('/', function(req, res, next) {
    ...
    for (let i = 0; i < templateLength; i++){
        ...
        let random = Math.floor(Math.random() * 100); //TODO randomized approach means we can sometimes get the same meme multiple times.
        img.id = templates[random].id;
        img.url = templates[random].url;
        memes.push(img);
    }
    res.send({
        code: 200,
